...
namespace AOC2020_7
{
    class Program
    {
        ...
        {
            string rawInput = @"""shiny purple bags contain 2 pale blue bags, 1 wavy fuchsia bag, 5 pale salmon bags.
...
dotted aqua bags contain 1 mirrored green bag, 5 shiny maroon bags.""";
            ...
            string[] splits = { "contain", "," };
            List<string> lookupTable = new List<string>();
            lookupTable.Add("shiny gold");
            int exitNum = 0, change = 0;
            bool logged = false;
            List<Tuple<int, int, string>> bagInfo = new List<Tuple<int, int, string>>();
            Tuple<int, int, string> data = new Tuple<int, int, string>(0, 1, "shiny gold");

            bagInfo.Add(data);

            //turn the list into key value pairs
            foreach (string bag in rows)
            {

                List<string> tempList = bag.Split(splits, StringSplitOptions.RemoveEmptyEntries).ToList();
                string key = tempList[0];
                tempList.RemoveAt(0);

                KeyValuePair<string, List<string>> pair = new KeyValuePair<string, List<string>>(key, tempList);

                dict.Add(pair.Key, pair.Value);
            }

            //While loop to add recursion
            while (exitNum == 0)
            {
                //saving table number to see if any changes happen in the loop, loop will exit if change = lookupTable
                //change = lookupTable.Count();

                //loop for each item in the lookuptable list
                foreach (string checks in lookupTable)
                {
                ...
                /*
                //for loop to go through the bag KV pairs
                foreach(KeyValuePair<string, List<string>> valuePair in dict)
                {
                    //check to see if bag was already added to list
                    logged = false;

                    for (int index = 0; index <= lookupTable.Count-1; index++){
                        if (valuePair.Key.Contains(lookupTable[index]))
                        {
                            logged = true;
                        }
                    }
                    ...
                    if (logged == false)
                    {
                        //loop for each Value bag in the pair
                        foreach (string bag in valuePair.Value)
                        {
                            //loop for each item in the lookuptable list
                            foreach (string checks in lookupTable)
                            {

                                if (bag.Contains(checks))
                                {
                                    lookupTable.Add(valuePair.Key.Substring(0, valuePair.Key.Length - 6));
                                    goto exitLoop;
                                }
                            }
                        }
